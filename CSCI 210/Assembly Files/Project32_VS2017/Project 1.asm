...
; Program Name:         Assembly Language Program Project 1 - Project 3
; Revision:             1.1
; Program Desciption:  
...
; loop to remove redundant code >>>> CHANGE
; if-else to help in removing code >>>> CHANGE
	 mov   ecx,COUNT

L1:	 mov   edx,OFFSET enternum		; copies enter number string into edx
.if  ecx == 1		;if statement checking if first or second user input
...
.endif
	 ...
	 call  WriteString		; writes to screen what's in edx
	 call  ReadInt		; reading in user input
	 mov   luckynumber, eax
	 call  Crlf		; adds a blank line

; creates a random number for computer favorite number
	 mov   eax,1000		; moves 1000 into eax
	 call  RandomRange		; creates a random number between 0-999
	 inc   eax		; adds 1 to eax
	 mov   ebx,luckynumber		; moves luckynumber into ebx

; if-else to check if computer has same lucky number >>>> CHANGE
.if  ebx == eax
	 mov   edx,OFFSET minetoo		; copies minetoo string into edx
.else
	 mov   edx,OFFSET mineis		; copies minetoo string into edx
.endif
	 call  WriteString		; writes to screen what's in edx
	 call  WriteInt		; writes to screen what's in eax
