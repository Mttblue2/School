...
public class SeatClient
{
	public static void main(String[] args) throws Exception
	{
		...
		try
		{
			ReservationMain ServerMain = (ReservationMain) r.lookup("reservationservice");
			String openServer = ServerMain.getOpenServer();
			System.out.println(openServer);
			ReservationServer Server = (ReservationServer) r.lookup(openServer);
			...
			while (!(content = keyboard.nextLine()).equalsIgnoreCase("Quit"))
			{
				...
				} catch (Exception e)
				{
					...
					openServer = ServerMain.getOpenServer();
					Server = (ReservationServer) r.lookup(openServer);
				}
			}
