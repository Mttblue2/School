public class MinHeap<E extends Comparable<E>>
{
	...
	public MinHeap()
	{
		heapArray = (E[])new Comparable[DEFAULT_CAP];
	}
	...
	private void downheap()
	{
		...
		while (LC < size && (heapArray[LC].compareTo(heapArray[index]) < 0 ||
							 heapArray[RC].compareTo(heapArray[index]) < 0))
		{
			...
			if (RC < size && heapArray[RC].compareTo(heapArray[LC]) < 0)
				smallest = RC;
			
			//check if the smallest child is less than the parent
			if (heapArray[smallest].compareTo(heapArray[index]) < 0)
				swap(smallest, index);
